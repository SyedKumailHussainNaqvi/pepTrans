...
def pepTrans_binary_classification(X, y, label_col, input_dim=2048, task='binary', pretrained_weights=None, test_csv_path=None):
    ...
    if pretrained_weights:
        ...
        with tf.device('/GPU:1'):
            ...
            try:
                original_df = pd.read_csv(test_csv_path)

                for i, prob in enumerate(test_preds):
                    pred_label = test_pred_labels[i]

                    sample_id = original_df["IDs"].iloc[i] if "IDs" in original_df.columns else f"sample_{i+1}"

                    if pred_label == 1:
                        result_text = "Interaction"
                    else:
                        result_text = "Non-interaction"

                    print(
                        f"{sample_id}: Prediction = {pred_label} "
                        f"({result_text}), Probability = {prob:.4f}"
                    )
                ...
                for i, prob in enumerate(test_preds):
                    ...
                    print(
                        f"sample_{i+1}: Prediction = {pred_label} "
                        f"({result_text}), Probability = {prob:.4f}"
                    )
...
def pepTrans_binding_site_prediction(h5_file_path, h5_test_file_path=None, pretrained_weights_path=None, n_splits=5):
    ...
    if pretrained_weights_path:
        ...
        with tf.device('/GPU:1'):
            ...
            try:
                seq_csv_path = "/home/kumail/Bioinformatics/github pepTrans/example_binding_site_sequences.csv"
                seq_df = pd.read_csv(seq_csv_path)

                for i in range(len(peptide_data)):
                    peptide_seq = str(seq_df["Peptide"].iloc[i])
                    sample_id = seq_df["IDs"].iloc[i] if "IDs" in seq_df.columns else f"sample_{i+1}"

                    pred_probs = predictions[i].flatten()[:len(peptide_seq)]
                    ...
                    if pred_labels.sum() > 0:
                        print("Overall peptide status: Interaction / contains predicted binding residues")
                    else:
                        print("Overall peptide status: Non-interaction / no predicted binding residues")

                    for pos, aa in enumerate(peptide_seq, start=1):
                        label = pred_labels[pos - 1]
                        prob = pred_probs[pos - 1]

                        if label == 1:
                            status = "Binding residue"
                        else:
                            status = "Non-binding residue"

                        print(f"Residue {pos:02d} {aa}: {status} | Probability = {prob:.4f}")

            except Exception as e:
                print(f"Could not print residue-level sequence output: {e}")
        ...
        with tf.device('/GPU:1'):
            ...
            try:
                ...
                for i, pred in enumerate(predictions):
                    ...
                    sample_id = (
                        original_df["IDs"].iloc[i]
                        if "IDs" in original_df.columns
                        else f"sample_{i+1}"
                    )

                    true_affinity = test_labels[i]

                    print(
                        f"{sample_id}: "
                        f"Predicted Binding Affinity = {pred:.4f} | "
                        f"True Binding Affinity = {true_affinity:.4f}"
                    )

            except Exception as e:

                print(f"Could not load sample IDs: {e}")
